...
function Random( prng, dtypes, dtype ) {
	...
	if ( !isFunction( prng ) ) {
		throw new TypeError( format( '1oF3c', prng ) );
	}
	...
	if ( !isStringArray( dtypes ) ) {
		throw new TypeError( format( '1oFAS', dtypes ) );
	}
	...
	if ( !contains( dtypes, dtype ) ) {
		throw new TypeError( format( '1oFFO', dtype ) );
	}
	this._prng = prng;
	this._dtypes = dtypes;
	this._dtype = dtype;
...
setReadOnly( Random.prototype, 'generate', function generate( len, options ) {
	...
	if ( !isNonNegativeInteger( len ) ) {
		throw new TypeError( format( '1oF2d', len ) );
	}
	opts = {};
...
setReadOnly( Random.prototype, 'assign', function assign( out ) {
	if ( !isCollection( out ) ) {
		throw new TypeError( format( '1oF2O', out ) );
	}
